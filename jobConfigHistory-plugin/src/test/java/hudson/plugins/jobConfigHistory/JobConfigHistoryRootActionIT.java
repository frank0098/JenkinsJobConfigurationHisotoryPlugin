...
public class JobConfigHistoryRootActionIT extends
    ...
    /**
     * Tests setting the new commit message and commit tags.
     */
    public void testSetCommitMessageAndTags() {
        // create some config history data

        String commitMessage = "THIS is A test!";
        String commitTags = "TEST,META";

        try {
            final HtmlPage htmlPage = webClient.goTo(JobConfigHistoryConsts.URLNAME + "/");
            HtmlTextInput msgField = (HtmlTextInput) htmlPage.getElementById("cmsg-field");
            HtmlTextInput tagsField = (HtmlTextInput) htmlPage.getElementById("ctags-field");

            msgField.setValueAttribute(commitMessage);
            tagsField.setValueAttribute(commitTags);

            final List<HtmlAnchor> anchors = htmlPage.getAnchors();
            for (HtmlAnchor anchor : anchors) {
                if (anchor.getTextContent().contains("Submit")) {
                    anchor.click();
                }
            }
            Thread.sleep(SLEEP_TIME);
            String fullCommitMessage = commitMessage + " [" + commitTags + "]";

            assertTrue("Verify commit message and tags reflect what was entered.", PluginUtils.getPlugin().getCommitString().equals(fullCommitMessage));
        } catch (Exception ex) {
            fail("Unable to complete testHistoryPage: " + ex);
        }
    }
    ...
    /**
     * Tests setting the both the new commit message and commit tags to the empty string.
     */
    public void testSetEmptyCommitMessageAndTags() {
        // create some config history data
