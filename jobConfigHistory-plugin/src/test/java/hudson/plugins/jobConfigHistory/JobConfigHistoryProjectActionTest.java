...
public class JobConfigHistoryProjectActionTest {
    ...
    /**
     * Test of setSyncStatus method, of class JobConfigHistoryProjectAction with parameter true.
     * @throws Exception
     */
    @Ignore
    @Test
    public void testSetSyncStatus() throws Exception {
        when(mockedProject.hasPermission(AbstractProject.CONFIGURE)).thenReturn(true);
    	JobConfigHistoryProjectAction sut = createAction();
    	sut.setSyncStatus(true);
    	assertTrue(mockedPlugin.getSyncStatusForProject(mockedProject.getName()));
    } 
    
    /**
     * Test of setSyncStatus method, of class JobConfigHistoryProjectAction with parameter false.
     * @throws Exception
     */
    
    @Test
    public void testSetSyncStatus2() throws IOException {
        when(mockedProject.hasPermission(AbstractProject.CONFIGURE)).thenReturn(true);
    	JobConfigHistoryProjectAction sut = createAction();
    	sut.setSyncStatus(false);
    	assertFalse(mockedPlugin.getSyncStatusForProject(mockedProject.getName()));
    }
    ...
    public void testGetSyncStatus() throws IOException {
        when(mockedProject.hasPermission(AbstractProject.CONFIGURE)).thenReturn(true);
    	JobConfigHistoryProjectAction sut = createAction();
    	sut.plugin.setSyncStatusForProject(mockedProject.getName(), true);
    	assertTrue(sut.getSyncStatus());
    }
    ...
    @Ignore
    @Test
    public void testGetSyncStatus2() throws IOException {
        when(mockedProject.hasPermission(AbstractProject.CONFIGURE)).thenReturn(true);
    	JobConfigHistoryProjectAction sut = createAction();
    	sut.plugin.setSyncStatusForProject(mockedProject.getName(), false);
    	assertTrue(sut.getSyncStatus());
    }
     ...
     */
    @Test
    public void testDoToggleSync() throws IOException {
    	when(mockedRequest.getParameter("timestamp")).thenReturn("2012-11-21_11-41-14");
    	JobConfigHistoryProjectAction sut = createAction();
    	sut.plugin.setSyncStatusForProject(mockedProject.getName(), true);
    	sut.doToggleSync(mockedRequest, mockedResponse);
    	assertFalse(sut.plugin.getSyncStatusForProject(mockedProject.getName()));
    }
